...
function getActiveSession($teacher_id)
{
    ...
    try {
        ...
        if (!$stmt)
            throw new Exception("DB error preparing active session check: " . $conn->error);
...
function createNewQRCode($session_id)
{
    ...
    try {
        ...
        if (!$verify_stmt)
            throw new Exception("DB error preparing session verify: " . $conn->error);
        ...
        do {
            ...
            if (!$check_stmt)
                throw new Exception("DB error preparing code uniqueness check: " . $conn->error);
        ...
        if (!$update_stmt)
            throw new Exception("DB error preparing old QR update: " . $conn->error);
        ...
        if (!$insert_stmt)
            throw new Exception("DB error preparing new QR insert: " . $conn->error);
...
function validateQRCode($code, $student_id)
{
    ...
    try {
        ...
        if (!$student_group_stmt)
            throw new Exception("DB error preparing student group fetch: " . $conn->error);
        ...
        if (!$find_stmt)
            throw new Exception("DB error preparing QR code fetch: " . $conn->error);
        ...
        if ($find_result->num_rows === 0) {
            ...
            if (!$check_stmt)
                throw new Exception("DB error preparing code check: " . $conn->error);
            $check_stmt->bind_param("s", $code);
            $check_stmt->execute();
            $check_result = $check_stmt->get_result();
        ...
        if (!$check_att_stmt)
            throw new Exception("DB error preparing attendance check: " . $conn->error);
        ...
        if (!$update_stmt)
            throw new Exception("DB error preparing QR update: " . $conn->error);
        ...
        if (!$record_stmt)
            throw new Exception("DB error preparing record insert: " . $conn->error);
